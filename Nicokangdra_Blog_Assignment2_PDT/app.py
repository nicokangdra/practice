...
def before_request():
    ...
    app.permanent_session_lifetime = 900    # session timeout in 900s or 15m
...
def index():
    ...
    cur = conn.cursor()
    sql = """
        SELECT art.id, art.title, usr.username
        FROM articles art
        JOIN users usr ON usr.id = art.user_id
    """
    cur.execute(sql)
    # [(1, "Article Title 1", "Username"), (2, "Title2", "Content 2"), ...]
...
def registerpage():
    if request.method == "POST" :
        ...
        # check if no same username it will run
        if not check :
            if password == passwordcon :
                cur = conn.cursor()
                sql = """INSERT INTO users (username, password, name) VALUES('%s','%s','%s')""" %(username,password,name)
                cur.execute(sql)
                conn.commit()
                cur.close()
                conn.close()
        ...
        # check if have same username will flash the message
        else:
